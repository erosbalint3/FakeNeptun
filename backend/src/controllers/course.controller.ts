...
const listCoursesForRegistration = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
const courseUsers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const response = await courseService.getCourseUsers(courseCode);
    res.status(200).json(response);
  } catch (error) {
    next(error);
  }
...
const saveCourseParticipations = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
